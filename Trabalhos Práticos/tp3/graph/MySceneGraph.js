   ...
   */
    parseComponents(componentsNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            var pickingIDIndex = nodeNames.indexOf("pickingID");
             
            if (pickingIDIndex != -1) {
                this.components[componentID].pickingID = this.reader.getInteger(grandChildren[pickingIDIndex], 'id')
            }
            ...
            for(j=transformationsNodes.length-1; j>=0; j--) {
                if(transformationsNodes[j].nodeName == "transformationref") {
                    ...
                    if (transId == null)
                        ...
                        mat4.multiply(this.components[componentID].transformMatrixInitial, this.components[componentID].transformMatrixInitial, transformation);
                    }
                    ...
                    mat4.translate(this.components[componentID].transformMatrixInitial, this.components[componentID].transformMatrixInitial, coordinates);
                    ...
                    mat4.rotate(this.components[componentID].transformMatrixInitial, this.components[componentID].transformMatrixInitial, axisAngle[1], axisAngle[0]);
                    ...
                    mat4.scale(this.components[componentID].transformMatrixInitial, this.components[componentID].transformMatrixInitial, scaleValues);
                }else {
    ...
    displayScene() {    
        ...
        for (let i=0; i<this.components; i++) {
            ...
            this.registerForPick(i+1, this.components[i]);
        }
    }

    /**
     * Sets location for StartMenu and for initial Camera according to vehicle's position.
     * */
    displayStartMenu() {
        var aux = this.scene.game.vehicle.position;
        var coordinate = [];

        coordinate[0] = aux[0]
        coordinate[1] = aux[1]
        coordinate[2] = aux[2] - 5
        
        // StartMenu position.
        mat4.translate(this.components["StartMenu"].transformMatrix, this.components["StartMenu"].transformMatrixInitial, coordinate);

        // Initial camera position.
        var fromCoordinate = [coordinate[0]-7, coordinate[1]+5, coordinate[2]+15]
        var toCoordinate = [coordinate[0], coordinate[1]+5, coordinate[2]]
        var cameraTmp = new CGFcamera(0.9, 0.2, 500, fromCoordinate, toCoordinate);
        this.scene.currentView.active = ""
        this.scene.camera = cameraTmp
        this.scene.interface.setActiveCamera(cameraTmp)
    }


    displayCar() {

        switch(this.scene.game.state) {
            case "demo":
                this.scene.game.vehicle.anim.animation(this.components["Car"]);
                break;
            case "game":
                this.scene.game.vehicle.anim.animation(this.components["Car"], this.components["wheel_front_left"], this.components["wheel_front_right"]);
                break;
            default:
                break;
        }
    ...
    displaySceneRecursive(componentID, materialParentID, textureParentID, toChangeMaterial) {
        ...
        if (componentID == "StartMenu" && this.scene.game.getGameStatus() >= 0) {
        } else {
            ...
            for (let i=0; i<currComponent.children.length; i++) {
                // set pick id before drawing
                if (currComponent.pickingID != null) {
                    this.scene.registerForPick(currComponent.pickingID, currComponent)
                } 
